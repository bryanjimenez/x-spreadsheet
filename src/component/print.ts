...
export default class Print {
  ...
  paper: { w:number, h:number, orientation:string, width:number, height:number, padding:number };
  data: DataProxy;
          ...
          .children(
            ...
            h('div', '-right').children(
              h('div', `${cssPrefix}-buttons`).children(
                ...
                new Button('next', 'primary').on('click', ()=>this.btnClick('next')),
              ),
            ),
          ),
            ...
            h('div', '-sider').child(
              h('form', '').children(
                ...
                h('fieldset', '').children(
                  ...
                  h('select', '').children(
                    ...PAGER_ORIENTATIONS.map((it, index) => h('option', '').attr('value', String(index)).child(`${t('print.orientations')[index]}`)),
  ...
  preview() {
    ...
    for (let i = 0; i < pages; i += 1) {
      ...
      if (scale < 1) draw.scale(scale, scale);
      const yof = yoffset;
      data.eachMergesInView(mViewRange, ({ sri, sci }: { sri:number, sci:number }) => {
