...
export default class ColorPalette {
    ...
    const table = h('table', '').children(
      h('tbody', '').children(
        h('tr', `${cssPrefix}-theme-color-placeholders`).children(
          ...themeColorPlaceHolders.map(color => this.buildTd(color)),
        ),
        ...themeColors.map(it => h('tr', `${cssPrefix}-theme-colors`).children(
          ...it.map(color => this.buildTd(color)),
        )),
        h('tr', `${cssPrefix}-standard-colors`).children(
          ...standardColors.map(color => this.buildTd(color)),
        ),
      ),
    );
    this.el.child(table);
  ...
  private buildTd(bgcolor:string) {
    return h('td', '').child(
