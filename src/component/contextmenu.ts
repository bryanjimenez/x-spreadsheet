...
export default class ContextMenu {
  ...
  private buildMenu() {
    return menuItems.map((it: (typeof menuItems)[number]) =>
      this.buildMenuItem(it)
    );
  }
  ...
  setPosition(x: number, y: number) {
    ...
    el.css("left", `${String(left)}px`);
    if (y > vhf) {
      el.css("bottom", `${String(view.height - y)}px`)
        .css("max-height", `${y}px`)
        .css("top", "auto");
    } else {
      el.css("top", `${String(y)}px`)
        .css("max-height", `${String(view.height - y)}px`)
        .css("bottom", "auto");
    }
    bindClickoutside(el);
  }
