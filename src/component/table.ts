...
export function renderCell(draw:Draw, data:DataProxy, rindex:number, cindex:number, yoffset = 0) {
  ...
  const style: ReturnType<DataProxy['defaultStyle']> = data.getCellStyleOrDefault(nrindex, cindex);
...
private renderContent(viewRange:CellRange, fw:number, fh:number, tx:number, ty:number) {
  ...
  draw.save();
  draw.translate(0, -exceptRowTotalHeight);
  data.eachMergesInView(viewRange, ({ sri, sci, eri }: { sri:number, sci:number, eri:number }) => {
...
private renderFixedHeaders(type:string, viewRange:CellRange, w:number, h:number, tx:number, ty:number) {
  const { draw, data } = this;
...
private renderContentGrid({
  sri, sci, eri, eci, w, h,
}:CellRange, fw:number, fh:number, tx:number, ty:number) {
  const { draw, data } = this;
  const { settings } = data;
