...
// fieldLabelWidth,
  ...
  constructor(key: SelectType, items:string[], width:string, getTitle:(it:unknown)=>string = it => it, change:(...arg:unknown[])=>void = () => {}) {
    this.key = key;
    this.getTitle = getTitle;
    this.vchange = () => {};
    this.el = h('div', `${cssPrefix}-form-select`);
    const f:Formula[] = items.map(i => ({ key: i, title: ()=>this.getTitle(i), render:()=>{}}));
    this.suggest = new Suggest(f, (item:Formula) => {

      this.itemClick(item.key as string);
      change(item.key);
