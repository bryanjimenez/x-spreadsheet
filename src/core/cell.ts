...
const infixExprToSuffixExpr = (src:string) => {
  const operatorStack = [];
  const stack = [];
...
const evalSubExpr = (subExpr:string, cellRender:Function) => {
  const [fl] = subExpr;
  let expr = subExpr;
  if (fl === '"') {
...
// formulaMap: {'SUM': {}, ...}
// cellRender: (x, y) => {}
const evalSuffixExpr = (srcStack:unknown[], formulaMap:Record<string,Formula>, cellRender:(x:number,y:number)=>string, cellList:string[]):string => {
  const stack:unknown[] = [];
 ...
 */
const cellRender = (src:string, formulaMap:Record<string,Formula>, getCellText:Function, cellList:string[] = []):string => {
  if (src[0] === '=') {
    ...
    return evalSuffixExpr(
      stack,
      formulaMap,
      (x:number, y:number) => cellRender(getCellText(x, y), formulaMap, getCellText, cellList),
      cellList,
    );
  }
  return src;
