...
class DrawBox {
  ...
  setBorders({
    ...
    top: DrawBox["borderTop"];
    bottom: DrawBox["borderBottom"];
    left: DrawBox["borderLeft"];
    right: DrawBox["borderRight"];
  }) {
...
class Draw {
  ...
  resize(width: number, height: number) {
    // console.log('dpr:', dpr);
    this.el.style.width = `${String(width)}px`;
    this.el.style.height = `${String(height)}px`;
    this.el.width = Draw.npx(width);
    this.el.height = Draw.npx(height);
  }
  ...
  translate(x: number, y: number) {
    this.ctx.translate(Draw.npx(x), Draw.npx(y));
    return this;
  }
  ...
  fillText(text: string, x: number, y: number) {
    this.ctx.fillText(text, Draw.npx(x), Draw.npx(y));
    return this;
  }

  private drawFontLine(
  ...
  */
  text(
    ...
    this.attr({
      textAlign: align,
      textBaseline: valign,
      font: `${font?.italic ? "italic" : ""} ${font?.bold ? "bold" : ""} ${String(Draw.npx(font.size))}px ${font.name}`,
      fillStyle: color,
      strokeStyle: color,
    });
    ...
    txts.forEach((it) => {
      ...
      if (textWrap && txtWidth > Draw.npx(biw)) {
        let textLine = { w: 0, len: 0, start: 0 };
        for (let i = 0; i < it.length; i += 1) {
          if (textLine.w >= Draw.npx(biw)) {
            ntxts.push(
              it.substring(textLine.start, textLine.start + textLine.len)
            );
  ...
  border(
    ...
    const { ctx } = this;
    ctx.lineWidth = Draw.thinLineWidth();
    ctx.strokeStyle = color;
    // console.log('style:', style);
    if (style === "medium") {
      ctx.lineWidth = Draw.npx(2) - 0.5;
    } else if (style === "thick") {
      ctx.lineWidth = Draw.npx(3);
    } else if (style === "dashed") {
      ctx.setLineDash([Draw.npx(3), Draw.npx(2)]);
    } else if (style === "dotted") {
      ctx.setLineDash([Draw.npx(1), Draw.npx(1)]);
    } else if (style === "double") {
      ctx.setLineDash([Draw.npx(2), 0]);
    }
    return this;
  }
  ...
  dropdown(box: DrawBox) {
    ...
    ctx.save();
    ctx.beginPath();
    ctx.moveTo(Draw.npx(sx), Draw.npx(sy));
    ctx.lineTo(Draw.npx(sx + 8), Draw.npx(sy));
    ctx.lineTo(Draw.npx(sx + 4), Draw.npx(sy + 6));
    ctx.closePath();
  ...
  error(box: DrawBox) {
    ...
    ctx.save();
    ctx.beginPath();
    ctx.moveTo(Draw.npx(sx - 8), Draw.npx(y - 1));
    ctx.lineTo(Draw.npx(sx), Draw.npx(y - 1));
    ctx.lineTo(Draw.npx(sx), Draw.npx(y + 8));
    ctx.closePath();
